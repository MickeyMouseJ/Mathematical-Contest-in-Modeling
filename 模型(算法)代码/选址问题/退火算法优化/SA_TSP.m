...
%% 初始解
S1=[randperm(CityNum,Need_City)];  %随机产生一个初始路线

%% 计算迭代的次数Time，即求解 T0 * q^x = Tend
Time=ceil(double(log(Tend/T0)/log(q)));

bestind=zeros(1,CityNum);      %历史最优个体
BestObjByIter=zeros(Time,1);       %每代目标值矩阵初始化

%% 迭代
while T0 > Tend
    count = count+1;     %更新迭代次数
    Population = zeros(L,Need_City); %为此温度下迭代个体矩阵分配内存
    ObjByIter = zeros(L,CityNum+1); %为此温度下迭代个体的目标函数值矩阵分配内存
    for k = 1:L
        %% 产生新解
        S2 = NewSolution(S1,CityNum);%交换
    ...
    %% 记录每次迭代过程的最优路线
    [d0,index] = min(ObjByIter(:,1)); %取出此温度下所有迭代中最优的一次
    if count == 1 || d0 < BestObjByIter(count-1) %若为第一次迭代或上次迭代比这次更满意
        BestObjByIter(count) = d0;            %如果当前温度下最优路程小于上一路程则记录当前路程
        bestind = Population(index,:);  %记录当前温度的最优地址
        bestLink=ObjByIter(index,2:end);
    else
...
bestroute = bestind; % 取得最优地址
best_link=bestLink;%连接
%% 输出最优解的路线和总距离
disp('-------------------------------------------------------------')
toc %显示运行时间
disp('最优地址');
...
%% 绘制最优解的实际路线
DrawPath(bestroute,City,best_link)