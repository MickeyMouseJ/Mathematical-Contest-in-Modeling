...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%网格分区
% optFis=genfisOptions('GridPartition');%网格分区
% optFis.NumMembershipFunctions = [ 2 2];%每列分别有2、2分级
% optFis.InputMembershipFunctionType = ["gaussmf" "gaussmf"];%第一二列隶属度函数为高斯函数
% initialfis=genfis(train_x,train_y,optFis);%生成网格分区模糊系统

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%减法聚类
optFis=genfisOptions('SubtractiveClustering');%减法聚类
optFis.ClusterInfluenceRange=[0.5];%聚类模糊变化
initialfis=genfis(train_x,train_y,optFis);%生成减法聚类模糊系统

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%C均值
% optFis = genfisOptions('FCMClustering','FISType','mamdani');
% opt.NumClusters = 1;%聚类个数
% initialfis=genfis(train_x,train_y,optFis);%生成减法聚类模糊系统


%命令的选项集anfis；'InitialFIS'，生成模糊系统；'EpochNumber'，迭代次数；'InitialStepSize'初始步长；'ValidationData'，验证数据
optAnFis = anfisOptions('InitialFIS',initialfis,'EpochNumber',300,'InitialStepSize',0.01,'ValidationData',valid_data);
...
%chkerror,均方根验证误差
[outfis,error,stepsize,chkfis,chkerror]=anfis(train_data,optAnFis);
...
% [x,mf] = plotmf(initialfis,'output',1);
% figure()
% plot(x,mf)
% %
...
fprintf('训练集：mse:%f; rmse:%f; mae:%f; sse:%f\n',train_mse,train_rmse,train_mae,train_sse);
%测试集
...
fprintf('测试集：mse:%f; rmse:%f; mae:%f; sse:%f\n',test_mse,test_rmse,test_mae,test_sse);
